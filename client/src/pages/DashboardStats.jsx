...
export default function DashboardStats({ dashboardData }) {
  ...
  useEffect(() => {
    ...
    if (dashboardData?.courses?.length) {
      ...
      const levelData = Object.keys(levelCounts).map(level => ({ name: level, count: levelCounts[level] }));
  ...
  useEffect(() => {
      ...
      coursesChartInstance.current = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: coursesByLevel.map((_, i) => COLORS[i % COLORS.length]),
      ...
      priceChartInstance.current = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: coursesByPrice.map((_, i) => COLORS[i % COLORS.length]),
            borderColor: 'white',
            borderWidth: 1,
            borderRadius: 4
          }]
      ...
      durationChartInstance.current = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: coursesByDuration.map((_, i) => COLORS[i % COLORS.length]),
            borderColor: 'white',
            borderWidth: 1
          }]
        },
        options: {
          ...
          maintainAspectRatio: false,
          cutout: '60%',
          plugins: { legend: { position: 'right', labels: { boxWidth: 12, padding: 15, font: { size: 11 } } }, tooltip: { callbacks: { label: ctx => { const val = ctx.raw; const total = ctx.dataset.data.reduce((s,v) => s+v,0); const pct = Math.round((val/total)*100); return `${val} courses (${pct}%)`; } } } }
        }
      });
    }
    ...
    // Cleanup on unmount
    return () => {
      [growthChartInstance, coursesChartInstance, priceChartInstance, durationChartInstance].forEach(ref => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
      ...
      <div className="bg-white p-6 rounded-xl shadow-md md:col-span-12">
        <h3 className="text-lg font-medium text-neutral-800 mb-4">Utilisateurs Actuels</h3>
        <div className="h-72"><canvas ref={growthChartRef} /></div>
      </div>

      {/* Courses by Level */}
      <div className="bg-white p-6 rounded-xl shadow-md md:col-span-6">
        <h3 className="text-lg font-medium text-neutral-800 mb-4">Courses by Level</h3>
        <div className="h-80"><canvas ref={coursesChartRef} /></div>
      </div>

      {/* Courses by Price */}
      <div className="bg-white p-6 rounded-xl shadow-md md:col-span-6">
        <h3 className="text-lg font-medium text-neutral-800 mb-4">Course Distribution by Price</h3>
        <div className="h-80"><canvas ref={priceChartRef} /></div>
      </div>

      {/* Courses by Duration */}
      <div className="bg-white p-6 rounded-xl shadow-md md:col-span-12">
        <h3 className="text-lg font-medium text-neutral-800 mb-4">Course Distribution by Duration</h3>
        <div className="h-72 flex justify-center"><canvas ref={durationChartRef} /></div>
      </div>
    </div>
  );
}
